...
void histogram(int* histogram, const int* buffer, const int size) {
   ...
   cudaMemset(histogram, 0, 256 * sizeof(int));
    ...
    histogram_kernel<<<grid_size, block_size>>>(histogram, buffer, size);

}


void histogram_equalization(int* buffer, const int* histogram, const int size, const int cdf_min) {
    const int block_size = BLOCK_SIZE(size);
    const int grid_size = (size + block_size - 1) / block_size;

    histogram_equalization_kernel<<<grid_size, block_size>>>(buffer, histogram, size, cdf_min);
}
